...
from src.utils import time_it, cleanup
from src.database import init_db, get_session
from src.storage import upload_blob
...
def video_editor(input_path: str, project_name: str) -> Tuple[Path, str] | None:
    ...
    try:
        ...
        logger.info(
            f"Successfully created {num_segments} video segments in {output_dir}"
        )
...
async def upload_to_db(project_name: str) -> str:
    ...
    try:
        ...
        download_link = await upload_blob(
            file_path, f"{project_name}_project_final_version.zip"
        )
...
def video_notifier(project_name: str, download_link: str | None = None):
    ...
    if download_link:
        ...
        emails = [
            "jemolife69@gmail.com",
            "omoparioladavidola@gmail.com",
            "heritageadewumivictor@gmail.com",
        ]
